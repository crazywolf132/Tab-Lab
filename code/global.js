...
var gist = {
	...
	save: function(options){
		...
		if (cssCode ||  (!creatingNew && gist.files && gist.files['lab.css'])) {
			files['lab.css'] = cssCode? { content: cssCode } : null;
		}

		if (htmlMarkup ||  (!creatingNew && gist.files && gist.files['lab.html'])) {
			files['lab.html'] = htmlMarkup? { content: htmlMarkup } : null;
		}

		if (jsCode || (!creatingNew && gist.files && gist.files['lab.js'])) {
			files['lab.js'] = jsCode? { content: jsCode } : null;
		}
	...
	load: function(id, rev){
		gist.request({
			...
			callback: function(data){
				...
				var cssFile = files['lab.css'],
					htmlFile = files['lab.html'],
					jsFile = files['lab.js'],
